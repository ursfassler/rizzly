...
public class CompInterfaceTypeChecker extends NullTraverser<Void, Void> {
  ...
  protected Void visitImplHfsm(ImplHfsm obj, Void param) {
    ...
    for( Transition tr : transList ) {
      Interface iface = tr.getEventIface().getLink();
      FunctionBase proto = iface.getPrototype().find(tr.getEventFunc());
      if( proto == null ) {
        RError.err(ErrorType.Error, tr.getInfo(), "interface " + iface.getName() + " has no function " + tr.getEventFunc());
      }
    }
