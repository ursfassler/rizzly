...
public class CompositionReduction extends NullTraverser<Named, Void> {
  ...
  public List<FunctionBase> genFunctions(List<Endpoint> conset, ListOfNamed<? extends FunctionBase> functions, Class<? extends FunctionBase> kindVoid, Class<? extends FunctionBase> kindRet) {
    ...
    for( FunctionBase fh : functions ) {
      ...
      ArrayList<NormalStmt> calls = new ArrayList<NormalStmt>();

      if( ptoto instanceof FuncWithReturn ) {
        assert ( conset.size() == 1 ); // typechecker should find this
        Endpoint con = conset.get(0);
        VarDefInitStmt call = makeQueryCall(con, (FuncWithReturn) impl);
        calls.add(call);
        body.getExit().setEnd(new ReturnExpr(info, new Reference(info, call.getVariable())));
      } else {
      ...
      body.insertCodeAfterEntry(calls, "BB_calls");
    }
    return ret;
  }

  static private VarDefInitStmt makeQueryCall(Endpoint ep, FuncWithReturn func) {
    ...
    SsaVariable var = new SsaVariable(info, "retval", func.getRet().copy());

    return new VarDefInitStmt(info, var, ref);
  }
