...
public class SubLinker_Test {
  ...
  public void uses_target_from_search() {
    ...
    Assert.assertEquals(result, ((LinkedAnchor) ref.getAnchor()).getLink());
  }
  ...
  public void removes_reference_offset() {
    ...
    Assert.assertEquals(0, ((ReferenceOffset) ref).getOffset().size());
  }
  ...
  private OffsetReference reference(Designator name) {
    ArrayList<String> list = name.toList();

    UnlinkedAnchor target = new UnlinkedAnchor(list.get(0));
    list.remove(0);
    ...
    return new OffsetReference(target, offset);
  }
}
