...
public class SingleAssignmentParser_Test {
  ...
  final private SingleAssignmentParser testee = new SingleAssignmentParser(stream, parser, error);
  final private InOrder order = Mockito.inOrder(stream, parser);

  @Test
  public void returns_itself_for_correct_name() {
    assertEquals(testee, testee.parserFor("SingleAssignment"));
  }
  ...
  public void returns_itself_for_correct_type() {
    assertEquals(testee, testee.parserFor(AssignmentSingle.class));
  }
  ...
  @Test
  public void parse_SingleAssignment() {
    Reference left = mock(Reference.class);
    Expression right = mock(Expression.class);
    when(parser.itemOf(Reference.class)).thenReturn(left);
    when(parser.itemOf(Expression.class)).thenReturn(right);

    AssignmentSingle assignment = testee.parse();

    assertEquals(left, assignment.getLeft());
    assertEquals(right, assignment.getRight());

    order.verify(stream).elementStart(eq("SingleAssignment"));
