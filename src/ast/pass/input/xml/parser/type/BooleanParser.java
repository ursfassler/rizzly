...
public class BooleanParser implements Parser {
  ...
  public BooleanParser(ExpectionParser stream, ObjectRegistrar objectRegistrar, XmlParser parser, RizzlyError error) {
    this.stream = stream;
  ...
  public Parser parserFor(Class<? extends Ast> type) {
    return type == BooleanType.class ? this : null;
  }

  @Override
  public BooleanType parse() {
    stream.elementStart(Name);
    String name = stream.attribute("name");
    String id = parser.id();
    stream.elementEnd();

    BooleanType object = new BooleanType(name);
